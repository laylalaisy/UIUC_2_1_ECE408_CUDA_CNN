...
#define w1_size 588
#define w2_size 14122

__constant__ float weight1[w1_size];
__constant__ float weight2[w2_size];
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    if (C == 1) {
      cudaMemcpyToSymbol(weight1, w.dptr_, w1_size * sizeof(float));
      forward_kernel<<<gridDim, blockDim, shared>>>(y.dptr_,x.dptr_,weight1, B,M,C,H,W,K);
    } else {
      cudaMemcpyToSymbol(weight2, w.dptr_, w2_size * sizeof(float));
      forward_kernel<<<gridDim, blockDim, shared>>>(y.dptr_,x.dptr_,weight2, B,M,C,H,W,K);
    }
